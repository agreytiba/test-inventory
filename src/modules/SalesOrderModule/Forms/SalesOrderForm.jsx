...
export default function SalesOrderFro({ subTotal = 0, current = null }) {
  ...
  return <LoadSalesOrderForm subTotal={subTotal} current={current} />;
}

function LoadSalesOrderForm({ subTotal = 0, current = null }) {
  const translate = useLanguage();
  const { last_invoice_number } = useSelector(selectFinanceSettings);
  const [total, setTotal] = useState(0);
  const [taxRate, setTaxRate] = useState(0);
  const [taxTotal, setTaxTotal] = useState(0);
  const [expiredDate, setCurrentYear] = useState(() => new Date().getFullYear());
